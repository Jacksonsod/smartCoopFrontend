...
const QualityInspectorDashboard = () => {
  ...
  const stats = useMemo(() => {
    ...
    return {
      ...
      pending: activities.filter(a => !a.qualityGrade && a.status !== "COMPLETED").length,
    };
  }, [activities]);
              ...
              <table className="min-w-full">
                <thead>
                  <tr className="border-b">
                    {["Date", "Member", "Item / Service", "Quantity", "Notes", "Status"].map(h => (
                  ...
                  {recentActivities.map(a => (
                    <tr key={a.id} className="hover:bg-gray-50 transition-colors">
                      ...
                      <td className="px-4 py-3 text-sm font-medium text-gray-900 whitespace-nowrap">
                        {a.memberName || a.member?.fullName || "-"}
                      </td>
                      ...
                      <td className="px-4 py-3 whitespace-nowrap">
                        <Badge className={a.qualityGrade ? "bg-emerald-50 text-emerald-700" : "bg-gray-100 text-gray-600"} variant="secondary">
                          {a.qualityGrade || a.status || "Pending"}
                        </Badge>
                      </td>
                    </tr>
                  ))}
